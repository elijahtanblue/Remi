...
This document is for implementation only. Product direction and scope live in [Remi-ticket-reconstruction-assistant-v3.md](../design/Remi-ticket-reconstruction-assistant-v3.md).

These instructions describe the current deployable v0.x system for operators and developers: Slack/Jira linking, event storage, `/brief`, Gmail sync, Confluence draft generation, admin review, and deployment.
...
3. When the Jira issue changes (status, assignee, priority) or new messages arrive in the linked thread, Remi regenerates the current summary/read
4. The current implementation surfaces that read via `/brief PROJ-123`, a legacy Slack overview, or an embedded Jira issue panel
5. Current `/brief` output is rules-based; the roadmap extends this into Current Work Records and reviewable action proposals
6. `/doc PROJ-123 handoff` assembles an issue-scoped operational brief and writes a draft Confluence page — timeline, key decisions, blockers, participants, linked threads, and email context all in one place

The legacy Slack overview and embedded Jira panel are current implementation surfaces. The long-term product home is the Remi web workflow, with Slack as an action surface and Jira as the issue anchor.

---

## What you'll need

These are internal/operator setup instructions for a guided pilot or private app install. They are not meant to position Remi as a broad self-serve customer install.
...
#### Recommended: use the installer welcome DM

After completing the guided Slack OAuth install (Step 12D), Remi sends the installer a direct message in Slack containing a **workspace-specific Jira descriptor URL** with your workspace ID already embedded. Use that URL — it looks like:
...
Remi can write operational briefs directly to Confluence pages via `/doc ISSUE-KEY [handoff|summary|escalation]`. In the current implementation this is an optional REST/OAuth setup step; strategically, Confluence draft generation is the first near-term docs target and remains write-only first. This requires an Atlassian OAuth 2.0 app.
...
- Departments — current v0.x grouping for Jira project prefixes and Slack channel patterns; roadmap isolation should use the more flexible `Scope` / `scopeId` primitive
...
## Current brief generation implementation

Current fallback briefs are generated by `packages/summary-engine` — rules-based, no LLM required.
...
**Scope isolation roadmap** — The current v0.x schema includes `Department` / `departmentId` as a grouping label for Jira project prefixes and Slack channel patterns. Future strategy should use `Scope` / `scopeId` instead because the boundary may be a team, workflow, project, department, or pilot rollout. Apply that primitive to `Issue`, `MemoryUnit`, future `IssueContextDocument`, future Confluence page records, and retrieval/query logic.
...
The connector architecture (Workspace → `*Install`) can extend, but strategy should stay anchored on the ticket reconstruction wedge:
- Confluence draft generation first — write-only REST/OAuth path, Slack-triggered generation, no macro/UI-heavy Confluence integration in V1
- Contextual uploads — issue-scoped pinned reference context, not authoritative truth and not an automatic override of live Slack/Jira/Gmail evidence
- Scope foundation — move future isolation and retrieval language toward `Scope` / `scopeId`, not team- or department-only boundaries
- Outlook/Exchange only after Gmail-backed workflows prove value
- Notion deferred until Confluence is validated
- Linear, GitHub Issues
- LLM prose rewriting only as a deliberate, cited V2 layer
